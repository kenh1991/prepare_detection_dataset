...
def convert(xml_list, json_file):
    ...
    for cate, cid in categories.items():
        cat = {'supercategory': 'liveness', 'id': cid, 'name': cate}
...
if __name__ == '__main__':
    ...
    for item in voc_path:
        name = item.split('/')[-1]
        print('mmmmmmmmmmmmmmmmmmmm',name)
        xml_dir = os.path.join(item , 'Annotations')
        save_json_train = './Annotations/train_' + name +'.json'
        save_json_val = './Annotations/val_' + name + '.json'
        save_json_test = './Annotations/test_' + name + '.json'
